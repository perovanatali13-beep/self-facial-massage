...
COURSE_PASSWORD=...         # общий код доступа (ручная выдача); персональные коды выдаются после оплаты
NEXT_PUBLIC_SITE_URL=https://self-facial-massage.vercel.app  # для Success/Notification URL и ссылок в письме
TINKOFF_TERMINAL_KEY=...    # эквайринг Т-Банк: Terminal Key
TINKOFF_PASSWORD=...        # эквайринг Т-Банк: пароль терминала
COURSE_PRICE_KOPECKS=390000 # цена курса в копейках (3900 ₽), опц.
RESEND_API_KEY=...          # отправка письма с кодом (Resend), опц.
EMAIL_FROM=Курс <noreply@ваш-домен>  # отправитель письма, опц.
```
...
- **Оплата и автовыдача доступа (Т-Банк + Resend):** форма покупки → `POST /api/pay/create`
  создаёт заказ в таблице `orders` (Supabase) и вызывает Т-Банк `Init`, редиректит на оплату.
  Webhook `POST /api/pay/notify` проверяет подпись и на `CONFIRMED` помечает заказ оплаченным,
  генерирует персональный код доступа (`SFM-XXXX-XXXX`) и шлёт его на email (Resend).
  Страница `/course/success?order=...` дополнительно подтверждает статус через `GetState`,
  показывает код и кнопку «Открыть курс» (ставит cookie доступа). Логика: `lib/payments.ts`
  (подпись SHA-256), `lib/orders.ts` (заказы/коды), `lib/email.ts`. Без ключей `/api/pay/create`
  отвечает 503 — сайт не падает. Notification URL в кабинете Т-Банк: `<site>/api/pay/notify`.
